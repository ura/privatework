...
public class NameUtil {
	...
	private static Pattern fileName = Pattern.compile("([A-Za-zＡ-Ｚａ-ｚ_\\.0-9#-_\\s]+\\.[A-Za-z0-9]+)");
	...
	public static String kan(String s) {
		...
		try {
			...
			System.out.println("巻数の取得できないファイルが存在しました。");
			System.out.println("どのように対応しますか？対応する場合は、文言を入力してください");
	...
	public static String createCominName(String name, Collection<File> list) {
		...
		if (size == 1) {
			...
			System.out.println("複数の著者候補がありました。");
			System.out.println("無視する場合は、ENTERを押してください");
	...
	public static String kan(Collection<File> list) {
		...
		for (File f : list) {
			...
			try {
				...
				System.out.println(e.getMessage());
				System.out.println("巻数の取得できないファイルが存在しました。");
				System.out.println("どのように対応しますか？無視する場合は、ENTERを押してください");
				System.out.println("入力を行った場合は、それがファイルの接尾子になります。（01-15巻＋画集）");
