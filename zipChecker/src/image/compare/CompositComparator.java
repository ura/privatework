...
public class CompositComparator implements ImgComparator {
	...
	private static Logger log = LoggerFactory.getLogger(ImgComparatorMiddle.class);
	...
	public void add( Img image2,int result){
		...
		if (result < sameLimit) {
			sameList.put(new SortKey<Integer, Img>(result, image2),
					image2);
		} else if (result < nearLimit) {
			nearList.put(new SortKey<Integer, Img>(result, image2),
					image2);
		}else{

			otherList.put(new SortKey<Integer, Img>(result, image2),
					image2);
		}
	...
	private void log(String msg,Img base, TreeMap<SortKey<Integer, Img>, Img> map) {
